  ...
  useEffect(() => {
    getAllProducts(urlApiProducts).then((data) => {
      ...
      const dataFilter = orderDate.filter(
        (element) =>
        element.name.substring(0, 3).toUpperCase() === "OSB");
      ...
      const variationFirst = (((numberOne - promedioOsb) / promedioOsb) * 100).toFixed(2)
      const numberTwo = Number(priceSecond.replace(/[$.]/g, "").trim())
      const variationSecond = (((numberTwo - promedioOsb) / promedioOsb) * 100).toFixed(2)
